...
//public Map<String, Object> Login(String id, String password, String uuid, String code);
    ...
    @Select("{ CALL LoginConfirm(#{uuid, jdbcType=CHAR, mode=IN}, #{code, jdbcType=VARCHAR, mode=IN}, #{id, jdbcType=VARCHAR, mode=IN}," +
            " #{password, jdbcType=VARCHAR, mode=IN}, #{result, jdbcType=INTEGER, mode=OUT}, #{account_id_out, jdbcType=VARCHAR, mode=OUT}," +
            " #{identity_out, jdbcType=INTEGER, mode=OUT}) }")
    @Options(statementType = StatementType.CALLABLE)
